...
// ==========================================
// 1. STRUKTUR DATA REKURSIF YANG DIJAMIN AMAN DARI TABRAKAN TYPE
// ==========================================
interface KuaNavbarItem {
  label: string;
  href: string;
  external?: boolean;
  submenu?: KuaNavbarItem[];
}

const menuItems: KuaNavbarItem[] = [
  { label: "Beranda", href: "/" },
  {
    label: "Profil",
    href: "#",
...
const Navbar = () => {
  ...
  // ==========================================
  const renderDesktopMenu = (items: KuaNavbarItem[] | undefined, level = 0) => {
    ...
    return (
      ...
      >
        {items.map((item: KuaNavbarItem, index: number) => {
          const hasSubmenu = !!(item.submenu && item.submenu.length > 0);
          return (
            <li key={index} className="relative group/item">
              {hasSubmenu ? (
                <>
                  <button className="flex w-full justify-between items-center px-5 py-2.5 text-white hover:bg-emerald-700 transition-colors duration-200 text-left">
    ...
    <header className="sticky top-0 z-50 w-full bg-gradient-to-r from-emerald-800 to-emerald-600 shadow-md">
      <div className="container mx-auto flex items-center justify-between px-4 py-3 lg:px-8 h-[70px]">
        ...
        <nav className="hidden lg:flex items-center gap-1.5 text-sm">
          {menuItems.map((item: KuaNavbarItem, i: number) => {
            const hasSubmenu = !!(item.submenu && item.submenu.length > 0);
            return hasSubmenu ? (
                  ...
                  {hasSubmenu ? (
                    <details className="group">
                      ...
                      <div className="flex flex-col pl-6 mt-1 mb-2 border-l-2 border-emerald-500/30 space-y-1 ml-4 transition-all duration-300">
                        {item.submenu?.map((sub: KuaNavbarItem, j: number) => {
                              ...
                              {hasSubSubmenu ? (
                                <details className="group/sub">
                                  ...
                                  <div className="flex flex-col pl-4 mt-1 border-l-2 border-emerald-500/20 space-y-1 ml-2">
                                    {sub.submenu?.map((subSub: KuaNavbarItem, k: number) => (
                                      <Link
                                        key={k}
                                        to={subSub.href}
